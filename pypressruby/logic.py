...
def remove_spike(y, x=None, nstd=2.0):
    if x is None:
        x = np.arange(len(y))
    rj = np.sqrt((x[:-1] - x[1:]) ** 2 + (y[:-1] - y[1:]) ** 2)
    ind = rj > np.median(rj) + nstd * np.std(rj)
    not_spikes = np.invert(np.hstack(([True], ind[:-1] * ind[1:], [True])))
    return (x[not_spikes], y[not_spikes])
...
def fit_data(x, y, xmin=None, xmax=None):
    ...
    if y is None:
        ...
        params["peak2_amplitude"].set(
            y[ymax_ind] * 0.5 * np.sqrt(2 * np.pi), min=0
        )
        ...
        params["peak1_amplitude"].set(
            y[ymax_ind] * 0.5 * np.sqrt(2 * np.pi) / 2, min=0
        )
